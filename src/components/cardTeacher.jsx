	...
	return (
		<>
			<div className="w-fit pl-5 pt-5">
				<div title="klik untuk melihat detail" onMouseMove={handdleMouse} onClick={() => document.getElementById(`detail-guru-${data.id}`).showModal()} className="group hover:-translate-y-1 hover:shadow-lg transition-[shadow_transform]  duration-300 cursor-pointer relative w-80 h-28 bg-base-300/30 rounded-xl">
					...
					<div className="pointer-events-none overflow-hidden rounded-[inherit] relative h-full">
						<div className="ml-20 pt-4 pr-6 mb-auto z-10">
							...
							<p className="text-current/50 text-sm">{data.jabatan}</p>
						</div>
			...
			<dialog id={`detail-guru-${data.id}`} className="modal">
				<div className="modal-box h-[37rem] flex flex-col items-stretch">
					<form method="dialog">
						<button className="fixed btn btn-sm btn-circle btn-ghost right-2 top-2">✕</button>
					</form>
