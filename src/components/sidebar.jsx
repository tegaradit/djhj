...
const Sidebar = ({ children }) => {
   ...
   const handdleChange = () => themeWeb == 'dark' ? setTheme('light') : setTheme('dark')
	...
	return (
		<>
         <nav className={`${sidebarOpen ? 'sm:left-64 sm:w-[calc(100%_-_256px)]' : 'sm:left-16 sm:w-[calc(100%_-_64px)]'} w-full transition-[left,_width] duration-300 flex fixed z-[9999] justify-between top-0 pl-8 sm:pl-0 pr-8 bg-base-300 box-border h-16 items-center`}>
			...
			<main>
				<aside className={`${sidebarOpen ? 'sm:w-64' : 'sm:w-16'} sm:block hidden fixed transition=[width] duration-300 h-screen left-0 top-0 bg-base-300 px-3 py-4 overflow-hidden`}>
				...
				<div className={`${sidebarOpen ? 'sm:ml-48' : 'sm:ml-0'} transition-[margin] duration-300 mt-16 p-0 pt-8 sm:p-12 sm:pl-28`}>
					{children}
				</div>
			</main>
		</>
