export default function ({ data }) {
	return (
		<div className="w-fit pl-5 pt-5">
			<div className="relative w-80 h-32 bg-base-300 rounded-xl flex flex-col justify-stretch items-stretch">
				<div className="absolute -translate-x-1/4 -translate-y-1/4 border-[5px] border-base-100 w-20 h-20 bg-gray-700 rounded-full">
					<img
						src={data.foto_profil}
						className="w-full h-full rounded-full object-cover"
					/>
				</div>
				<div className="ml-20 pt-4 pr-6 mb-auto">
					<h1 className="font-bold">{data.nama}</h1>
					<p className="text-white/50 text-sm">{data.jabatan}</p>
				</div>

				<button
					onClick={() =>
						document.getElementById(`detail-guru-${data.id}`).showModal()
					}
					style={{
						height: "1.5rem",
						minHeight: "0rem",
					...
					className="mr-3 mb-3 self-end btn btn-outline btn-primary w-14"
				>
					details
				</button>
				<dialog id={`detail-guru-${data.id}`} className="modal">
					<div className="modal-box h-[37rem] flex flex-col items-stretch">
						...
						<div className="flex flex-col items-center self-center p-6">
							<h1 className="font-bold text-lg mb-4">{data.nama}</h1>
							<div className="avatar mb-4">
								<div className="w-32 rounded-full">
									<img src={data.foto_profil} />
								</div>
							</div>
							<h2 className="text-white/50">8888-8888-8888</h2>
							<p className="font-bold text-center">{data.mapel}</p>
						</div>
