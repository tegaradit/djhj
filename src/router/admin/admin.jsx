...
const dataAddProject = {
	...
	changeLinkProject: function (value) {this.linkProject = value},
	changeJudulProject: function (value) {this.judulProject = value},
	changeDescription: function (value) {this.description = value},
	changeThumbnail: function (value) {this.thumbnail = value},
	changeCreator: function (value) {this.creator = value},
	changeClass: function (value) {this.class = value}
}
...
const Addprojects = () => {
   const [theme, _] = useContext(AppContext).theme
	...
	const 
		...
		[judul, setJudul] = useState(),
		[description, setDescription] = useState(),
		[thumbnail, setThumbnail] = useState(),
		[creator, setCreator] = useState(),
		[class_, setClass_] = useState()

	const handdleSubmit = (e) => {
		e.preventDefault()

		try {
			fetch('https://api.pplgsmenza.id/projek', {
				method: 'post',
				headers: {
				...
				body: JSON.stringify({
					"nama_project": "judul",
					"pembuat_project": "creator",
					"kelas": "class_",
					"deskripsi_project": "description",
					"link_project": "link",
					"upload_at": "toUTCString",
					"gambar": "thumbnail"
				})
			})
		} catch (e) {
	...
	return (
		<div className="max-w-4xl w-full">
			...
			<form onSubmit={handdleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-y-6 md:gap-x-10">
				<div>
					<label className="block text-base-content/90 text-sm" htmlFor="link-project">Link Project</label>
					<input value={link} onChange={e => setLink(e.target.value)} className="w-full px-2 py-1 rounded-lg" required type="text" name="link-project" />
				</div>

				<div>
					<label className="block text-base-content/90 text-sm" htmlFor="judul-project">Judul Project</label>
					<input value={judul} onChange={e => setJudul(e.target.value)} className="w-full px-2 py-1 rounded-lg" required type="text" name="judul-project" />
				</div>

				<div>
					<label className="block text-base-content/90 text-sm" htmlFor="deskripsi-project">Deskripsi Project</label>
					<textarea value={description} onChange={e => setDescription(e.target.value)} className="w-full p-2 resize-none rounded-lg" rows={8} required type="text" name="deskripsi-project" />
				</div>

				<div className="h-full">
					<label className="block text-base-content/90 text-sm" htmlFor="thumnail-project">Thumnail Project</label>
					<InputImage stateSetter={img => setThumbnail(img)} className={`${theme == "dark" ? "bg-[#3B3B3B]" : "bg-white"} overflow-hidden relative w-full h-[24.1ch] leading-[24.1ch] text-center cursor-pointer rounded-lg`} />
				</div>

				<div>
					<label className="block text-base-content/90 text-sm" htmlFor="nama-pembuat">Nama Pembuat</label>
					<input value={creator} onChange={e => setCreator(e.target.value)} className="w-full px-2 py-1 rounded-lg" required type="text" name="nama-pembuat" />
				</div>

				<div>
					<label className="block text-base-content/90 text-sm" htmlFor="kelas">Kelas</label>
					<select value={class_} onChange={e => setClass_(e.target.value)} className="rounded-lg px-2 py-1 w-full" name="kelas">
	...
	return (
		<div className="max-w-4xl w-full">
			...
			<Form method="post" className="grid grid-cols-1 md:grid-cols-2 gap-y-6 md:gap-x-10">
				...
				<div className="h-full">
					<label className="block text-base-content/90 text-sm" htmlFor="thumnail-project">Thumnail Project</label>
					<InputImage stateSetter={setImage} className={`${theme == "dark" ? "bg-[#3B3B3B]" : "bg-white"} overflow-hidden relative w-full h-[24.1ch] leading-[24.1ch] text-center cursor-pointer rounded-lg`} fallbackImagePreview={data.thumbnail} />
				</div>
